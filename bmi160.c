...
void bmi160_device_config(void) {
...
void bmi160_nrf5_init(uint8_t ACC_ODR, uint8_t GYR_ODR) {
  ...
  device.gyro_cfg.range = BMI160_GYRO_RANGE_2000_DPS;
...
void bmi160_fifo_config(const uint16_t fifo_buffer_len) {
  ...
  result = bmi160_set_fifo_config(BMI160_FIFO_GYRO | BMI160_FIFO_ACCEL |
                                      BMI160_FIFO_HEADER | BMI160_FIFO_TIME,
                                  BMI160_ENABLE, &device);
  if (result == BMI160_OK) {
